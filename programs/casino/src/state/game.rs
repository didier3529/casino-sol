...
use super::{GameType, GameStatus, GameResult, ProvableFairData};
...
impl Game {
    ...
    pub fn verify_provable_fairness(&self, server_seed: &str) -> Result<bool> {
        // Using Solana's native hashing (sha256) instead of sha2 crate
        use solana_program::hash::hash;
        
        // Verify server seed hash using Solana's hash function
        let computed_hash = hash(server_seed.as_bytes());
        let computed_hash_str = computed_hash.to_string();
        
        Ok(computed_hash_str == self.provable_fair.server_seed_hash)
    }

    pub fn generate_game_outcome(&self, server_seed: &str) -> Result<Vec<u8>> {
        // Using Solana's native hashing instead of sha2 crate
        use solana_program::hash::hash;
        ...
        let hash_result = hash(combined_seed.as_bytes());
        let hash_bytes = hash_result.to_bytes();
        
        // Generate outcome based on game type
        match self.game_type {
            GameType::CoinFlip => {
                let result = hash_bytes[0] % 2;
                Ok(vec![result])
            },
            GameType::DiceRoll => {
                let result = (hash_bytes[0] % 100) + 1;
                Ok(vec![result])
            },
            GameType::Slots => {
                // Generate 3 reels
                let reel1 = hash_bytes[0] % 10;
                let reel2 = hash_bytes[1] % 10;
                let reel3 = hash_bytes[2] % 10;
                Ok(vec![reel1, reel2, reel3])
            },
            GameType::Roulette => {
                let result = hash_bytes[0] % 37; // 0-36 for European roulette
                Ok(vec![result])
            },
            _ => {
                // For more complex games, implement specific logic
                Ok(hash_bytes[0..4].to_vec())
            }
        }
    }
